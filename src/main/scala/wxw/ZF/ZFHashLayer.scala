...
//    jedis.close()
  ...
  /**
    *
    * @param data   需要计算压缩点的所有原始点
    * @param indexs   需要降维的属性的索引
    * @param tempUpperBound   每个压缩点对应的原始点个数
    *        itqRatioN   原始点的像素中抽取 1/itqRatioN 的点作为 IncreSVD 的训练点
    * @return
    */
  def hashOnce(data: Array[LabeledPoint], indexs: Array[Int], tempUpperBound: Double): Array[Array[Int]] = {
    ...
    var numFeatures = data.last.features.size   // 单张图片的像素点数
    var numInc = indexs.size

    val tempItqBitN = math.log(indexs.size / tempUpperBound) / math.log(2)   // 需要分桶的次数，也是降维后目标属性个数
    ...
    System.out.println(s"incremental svd cost: ${(System.nanoTime() - incStart) / 1e9 } seconds")
    ...
    System.out.println(s"split cost: ${(System.nanoTime() - splitStart) / 1e9} seconds")
