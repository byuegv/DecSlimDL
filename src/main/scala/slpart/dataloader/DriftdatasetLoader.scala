...
object DriftdatasetLoader {
  def load(dirPath: String,labOrConstr: Boolean = true,sc: SparkContext = null) = {
    ...
    val parsed = sc.textFile(dirPath + "/*").map(_.trim)
      ...
      .map{line =>{
        ...
        val label = if(labOrConstr) lorc.head.toDouble
        else lorc.last.toDouble
