...
object OptionUtils {
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args,new TrainParams()).map(param => {
      System.out.println(parseLabelSelection2Set(param.labelSelection))
...
//      System.out.println(s"ksubsampleSize: ${param.ksubsampleSize}")
  ...
  case class TrainParams(
                        ...
                        hiddenNodes: Option[Int] = None,

                        redisHost: String = "localhost",
                        redisPort: Int = 6379,
                        redisDatabase: Int = 0,
                        redisSecret: Option[String] = None,
                        redisClusterMode: Boolean = false,
                        fixedCriticalRatio: Option[Double] = None,
                        edgeNum: Int = 2,
                        aggMethod: String = "ISVDLSH",
                        labelSelection: String = "1-10",
                        sampleFraction: Option[Double] = None,
                        epochInterval: Int = 1,
                        exchangeProb: Double = 0.05
                        )
  val trainParser = new OptionParser[TrainParams]("train option parser") {
    ...
    opt[Boolean]("fullGradients")
      .text("to use full gradients for remove")
      .action((x,c) => c.copy(fullGradients = x))
    ...
    opt[Int]("hiddenNodes")
      .text("hiddenNodes of singler layer regression")
      .action((x,c) => c.copy(hiddenNodes = Some(x)))

    opt[String]("redisHost")
      .text("redis host ip")
      .action((x,c) => c.copy(redisHost = x))
    opt[Int]("redisPort")
      .text("redis host port")
      .action((x,c) => c.copy(redisPort = x))
    opt[Int]("redisDatabase")
      .text("redis database index")
      .action((x,c) => c.copy(redisDatabase = x))
    opt[String]("redisSecret")
      .text("redis auth passwd")
      .action((x,c) => c.copy(redisSecret = Some(x)))
    opt[Boolean]("redisClusterMode")
      .text("redis host ip")
      .action((x,c) => c.copy(redisClusterMode = x))
    opt[Double]("fixedCriticalRatio")
      .text("redis host ip")
      .action((x,c) => c.copy(fixedCriticalRatio = Some(x)))
    opt[Int]("edgeNum")
      .text("the number of edges")
      .action((x,c) => c.copy(edgeNum = x))
    opt[String]("aggMethod")
      .text("the method for generate aggregation points: ISVDLSH,HKMEANS,SVDKMEANS")
      .action((x,c) => c.copy(aggMethod = x))
    opt[String]("labelSelection")
      .text("select training data based on labelSelection, examples: 1-10 or 2,3,4 or 1-5,7,9")
      .action((x,c) => c.copy(labelSelection = x))
    opt[Double]("sampleFraction")
      .text("rdd.sample fraction between [0,1.0]")
      .action((x,c) => c.copy(sampleFraction = Some(x)))
    opt[Int]("epochInterval")
      .text("every epochInterval select critical part of trining data")
      .action((x,c) => c.copy(epochInterval = x))
    opt[Double]("exchangeProb")
      .text("The probability of exchange model parameters")
      .action((x,c) => c.copy(exchangeProb = x))
  }
  ...
  case class TestParams(
                       ...
                       storeQuantizedModel: Boolean = true,
                       quantizedModel: Option[String] = None
                       )
  val testParser = new OptionParser[TestParams]("test option parser") {
    opt[String]('f',"dataset")
      ...
      .action((x,c) => c.copy(dataset = x))
    opt[Boolean]("zeroScore")
      .text("wether to zero test dataset")
      .action((x,c) => c.copy(zeroScore = x))
    opt[String]("model")
      .text("path of model")
    ...
    opt[Boolean]("quantize")
      .text("wether to quantize test model")
      .action((x,c) => c.copy(quantize = x))
    opt[Boolean]("storeQuantizedModel")
      .text("if store quantized Model")
      .action((x,c) => c.copy(storeQuantizedModel = x))
    opt[String]("quantizedModel")
      .text("where to store quantized model")
      .action((x,c) => c.copy(quantizedModel = Some(x)))
  }
  ...
  def parseLabelSelection2Set(labelSelection: String) = {
    val lines = labelSelection.trim().split(",")
    var results = Set[Int]()
    for(line <- lines){
      if(line.contains("-")){
        val inli = line.trim.split("-")
        require(inli.length == 2,"the label range should with a start and an end")
        val start = inli.head.toInt
        val end = inli.last.toInt
        for(elem <- start to end){
          results = results + elem
        }
      }
      else{
        results = results + line.trim.toInt
      }
    }
    results
  }
