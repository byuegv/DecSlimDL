...
import slpart.models.archsets.squeezenet.SqueezeNetCifarV2

object TrainSqueezeNetV2 {
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args,new TrainParams()).map(param =>{
      ...
      val model = if(param.loadSnapshot && param.model.isDefined){
        ...
        SqueezeNetCifarV2(numClass = param.classes,hasBN = param.hasBN,hasDropout = param.hasDropout)
      }
