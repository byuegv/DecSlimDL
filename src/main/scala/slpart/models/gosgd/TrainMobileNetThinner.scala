  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args,new TrainParams()).map(param =>{
      ...
      val model = if(param.loadSnapshot && param.model.isDefined){
        ...
        MobileNetThinner(param.classes,hasDropout = param.hasDropout,hasBN = param.hasBN,
          shallow = param.shallow,depthMultiplier = param.depthMultiplier)
      }
