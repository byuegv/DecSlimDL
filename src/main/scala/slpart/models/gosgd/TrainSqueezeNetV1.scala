...
import slpart.models.archsets.squeezenet.SqueezeNetCifarV1

object TrainSqueezeNetV1 {
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args,new TrainParams()).map(param =>{
      ...
      val model = if(param.loadSnapshot && param.model.isDefined){
        ...
        SqueezeNetCifarV1(numClass = param.classes,hasBN = param.hasBN,hasDropout = param.hasDropout)
      }
