package slpart.models.gossipgrad.lenet

import java.text.SimpleDateFormat
import java.util.Date

import com.intel.analytics.bigdl.dataset.{DataSet, DistributedDataSet, Identity, Sample}
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args,new TrainParams()).map(param =>{
      ...
      val trainSamples = if(param.sampleFraction.isDefined){
        AggMNIST.trainMnistData(sc,labelSel,aggMethod = param.aggMethod,path = param.dataset, upBound = param.upBound)
      ...
      val trainDataset = (DataSet.rdd(trainSamples.zipWithUniqueId().map(x => (x._2,x._1))) -> Identity())
        .asInstanceOf[DistributedDataSet[(Long,Array[Sample[Float]])]]

      val criterion = ClassNLLCriterion[Float]()
      val optimizer = new DecGossipGradDistriOptimizer(model,trainDataset,_criterion = criterion)
      ...
      remvControler.isAggregatedSamples = param.isAggregatedSamples
      remvControler.isTrainWithAggregatedSamples = param.isTrainWithAggregatedSamples
      ...
      optimizer.setValidation(
        ...
        vMethods = Array(new Top1Accuracy(),new Top5Accuracy(),new Loss(criterion = ClassNLLCriterion[Float]())),
        batchSize = param.batchSize
      )
      optimizer.setOptimMethod(optimMethod)
      optimizer.setEndWhen(Trigger.maxEpoch(param.maxEpoch))

      if(param.summary.isDefined){
        val logdir = param.summary.get
        val appName = s"${curTime}-${param.appName}"
        val trainSummary = TrainSummary(logdir,appName)
        val validationSummary = ValidationSummary(logdir,appName)
        trainSummary.setSummaryTrigger(tag = "Loss",Trigger.everyEpoch)
        trainSummary.setSummaryTrigger(tag = "Parameters",Trigger.everyEpoch)
