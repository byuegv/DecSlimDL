...
import slpart.dataloader.TinyLoader
...
object STINYIMAGE {
  ...
  def main(args: Array[String]): Unit = {
    trainParser.parse(args,new TrainParams()).map(param =>{
      ...
      val trainSamples = TinyLoader.trainingSamplesAry(sc,param.dataset,param.zeroScore,isAggregate = param.aggregate,category = param.classes,
      ...
      val validationSamples = TinyLoader.validateSamples(sc,param.dataset,param.zeroScore,category = param.classes)
      ...
      val trainOut = new PrintWriter("tiny-training.txt")
      trainSamples.map(_._2).collect().foreach(agglist => {
        ...
        for(i <- 0 until maxlen){
          ...
          val feature = point.feature().reshape(Array(1*32*32)).squeeze().toArray()
      ...
      trainOut.close()
      val testOut = new PrintWriter(("tiny-test.txt"))
