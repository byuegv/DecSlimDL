...
/**
  ...
  */
private[ann] trait ZFLossFunction {
  /**
...
private[ann] class ZFSigmoidLayerWithSquaredError extends ZFLayer {
  override val weightSize = 0
  override val inPlace = true

  override def getOutputSize(inputSize: Int): Int = inputSize
  override def createModel(weights: BDV[Double]): ZFLayerModel =
    new ZFSigmoidLayerModelWithSquaredError()
  override def initModel(weights: BDV[Double], random: Random): ZFLayerModel =
    new ZFSigmoidLayerModelWithSquaredError()
}

private[ann] class ZFSigmoidLayerModelWithSquaredError
  extends ZFFunctionalLayerModel(new ZFFunctionalLayer(new ZFSigmoidFunction)) with ZFLossFunction {
...
private[ann] class ZFSoftmaxLayerWithCrossEntropyLoss extends ZFLayer {
  override val weightSize = 0
  override val inPlace = true

  override def getOutputSize(inputSize: Int): Int = inputSize
  override def createModel(weights: BDV[Double]): ZFLayerModel =
    new ZFSoftmaxLayerModelWithCrossEntropyLoss()
  override def initModel(weights: BDV[Double], random: Random): ZFLayerModel =
    new ZFSoftmaxLayerModelWithCrossEntropyLoss()
}

private[ann] class ZFSoftmaxLayerModelWithCrossEntropyLoss extends ZFLayerModel with ZFLossFunction {
