...
/**
  ...
  */
class ZFZipLenet5(layerBuilder: CNNTopology, hiddenN: Int, outN: Int, inN: Int = 1) extends Serializable with Logging {
  ...
  def train(trainSet: RDD[(LabeledPoint, Array[LabeledPoint])], testSet: RDD[LabeledPoint], foldN: Int) {
    var t = 1
    val trainSize = trainSet.count().toInt
    val temp = train(trainSet.first._1)
    val gZero = temp._2
    ...
    var accitTime = 0L
    val zipN = trainSet.sparkContext.longAccumulator
    val pointsN = trainSet.sparkContext.longAccumulator
    var afterFilterPointsN = 0

    val zipTime = trainSet.sparkContext.longAccumulator
    val pointTime = trainSet.sparkContext.longAccumulator
    ...
    while (t <= maxIterations) {
      ...
      for (f <- 0 until foldN) {
        ...
        val selectData = splits(f).mapPartitions(pit => {
          if (pit.isEmpty) {
            pit.flatMap(_._2)
          } else {
            val partT = System.currentTimeMillis()
            val (p1, p2) = pit.duplicate
            val zipGrads: Array[Double] = p1.map(t => {
              val zgrad: Array[(Array[Array[BDM[Double]]], Array[Double])] = trainzip(t._1)._2
              val zg1 = zgrad(1)._1.map(ms => ms.map(m => m.toArray.map(a => a * a).sum).sum).sum
              //            gAcc.add(zg1)
              pointsN.add(t._2.size)
              zg1
            }).toArray //计算压缩点的gradient,以此作为排序标准
            zipN.add(zipGrads.size)
            val zgmean = zipGrads.sum / zipGrads.size
            //          val gi = zipGrads.zipWithIndex.sortWith(_._1 > _._1).map(_._2).slice(0, (ratio * zipGrads.size).toInt)
            //          p2.zipWithIndex.filter(t => gi.contains(t._2)).flatMap(_._1._2)
            zipTime.add(System.currentTimeMillis() - partT)
            p2.zip(zipGrads.iterator).filter(t => t._2 > zgmean).flatMap(_._1._2) //筛选符合条件的压缩点

          }
        })
        val (gradientSum, right, count, fullGradSum) = selectData.repartition(trainSet.partitions.size)
          //        .sample(false, batchSize.toDouble / trainSize, 42 + t)
          .treeAggregate((gZero, 0, 0, fullZero))(
          seqOp = (c, v) => {
            val partTime = System.currentTimeMillis()
            val result = train(v)
            ...
            pointTime.add(System.currentTimeMillis() - partTime)
            (ZFZipLenet5.combineGradient(c._1, gradient), c._2 + right, c._3 + 1, c._4.zip(fullGradient).map(t => t._1 + t._2))
          },
          combOp = (c1, c2) => {
            (ZFZipLenet5.combineGradient(c1._1, c2._1), c1._2 + c2._2, c1._3 + c2._3, c1._4.zip(c2._4).map(t => t._1 + t._2))
          })
        afterFilterPointsN += count
        ...
        if (count > 0) {
          ...
          totalRight += right
//          if (t % 10 == 0 && f == 0) {
//            println("precision" + totalRight.toDouble / totalCount)
//          }
        }
      ...
      if (t % 1 == 0) {
        ...
        println(t + ",ACC," + ACC + ",itTime," + accitTime + ",zipPartTime, " + zipTime.value + ",pointPartTime," + pointTime.value + ",zipN," + zipN.value + ",pointN," + pointsN.value + ",afterFilterPointsN," + afterFilterPointsN + ",precision," + totalRight.toDouble / totalCount)
      }
      t += 1
  ...
  private def train(record: LabeledPoint): (Boolean, Array[(Array[Array[BDM[Double]]], Array[Double])], Array[BDM[Double]]) = {
    ...
    val right = if (ZFZipLenet5.getMaxIndex(out2.toArray) == ZFZipLenet5.getMaxIndex(label.toArray)) true else false
  ...
  private def trainzip(record: LabeledPoint): (Boolean, Array[(Array[Array[BDM[Double]]], Array[Double])], Array[BDM[Double]]) = {
    val outputs = forward(record.features)
    ...
    val delta1 = ZFFullConnection.getDelta(lastOut, weights1, delta2)
    val errors = backPropagation(delta1.toArray.map(d => new BDM[Double](1, 1, Array(d))), outputs)
    val gradient = getGradientZip(outputs, errors)
    (right, gradient, Array(grad1, gbais1))
  }
  ...
  private def getGradientZip(
                              outputs: Array[Array[BDM[Double]]],
                              errors: Array[Array[BDM[Double]]]): Array[(Array[Array[BDM[Double]]], Array[Double])] = {
    var l: Int = 1
    val gradient = new Array[(Array[Array[BDM[Double]]], Array[Double])](layerNum)
    while (l <= 1) {
      // gradient 1,3,5 has value
      val layer: CNNLayer = layers.get(l)
      val lastLayer: CNNLayer = layers.get(l - 1)
      gradient(l) = layer.getType match {
        case "conv" =>
  ...
  private def getKernelsGradient(
    ...
    while (j < mapNum) {
      ...
      while (i < lastMapNum) {
        ...
        val deltaKernel = ZFZipLenet5.convnValid(lastOutput(i), error)
  ...
  private def setSampErrors(
    ...
    while (i < mapNum) {
      ...
      while (j < nextMapNum) {
        ...
        if (sum == null) {
          sum = ZFZipLenet5.convnFull(nextError, flipud(fliplr(kernel)))
        }
        else {
          sum += ZFZipLenet5.convnFull(nextError, flipud(fliplr(kernel)))
        }
  ...
  private def setConvErrors(
    ...
    while (m < mapNum) {
      ...
      outMatrix = outMatrix :* ZFZipLenet5.kronecker(nextError, scale)
      errors(m) = outMatrix
      m += 1
    }
    errors
  ...
  private def setOutLayerErrors(
    ...
    val outClass = ZFZipLenet5.getMaxIndex(outValues)
  ...
  private def setConvOutput(
    ...
    while (j < mapNum) {
      ...
      while (i < lastMapNum) {
        ...
        if (sum == null) {
          sum = ZFZipLenet5.convnValid(lastMap, kernel)
        }
        else {
          sum += ZFZipLenet5.convnValid(lastMap, kernel)
        }
  ...
  private def setSampOutput(
    ...
    while (i < lastMapNum) {
      ...
      output(i) = ZFZipLenet5.scaleMatrix(lastMap, scaleSize)
      i += 1
    }
    output
  }
...
object ZFZipLenet5 {
  ...
  def main(args: Array[String]) {
    ...
    // /Users/zhangfan/Downloads/mnist/train.txt /Users/zhangfan/Downloads/mnist/test.txt 1000 60 6 784 true 1 10 10 5 locahost
    ...
    val numIt = args(2).toInt //5000 epoch次数
    val batchSize = args(3).toInt //16 每次抽取实例个数
    val minPart = args(4).toInt //6 最小Part个数
    val numFeature = args(5).toInt //属性个数
    val isSparse = args(6).toBoolean //输入数据是否为libsvm格式
    val inputN = args(7).toInt //channel个数


    val itqitN = args(8).toInt //压缩算法迭代次数
    val itqratioN = args(9).toInt //压缩算法每itqratioN个属性中使用一个属性进行计算
    val itqBoundN = args(10).toInt //每个压缩点包含原始点个数上限
    val redisHost = args(11) //启动redis的主机ip

    val foldN = args(12).toInt //每个epoch包含多少个小的迭代

    println("dataPath, " + dataPath + ",foldN, " + foldN + ",numIt," + numIt + ",batchSize," + batchSize + ",minPart," + minPart + ",numFeature," + numFeature + ",isSparse," + isSparse + ",inputN," + inputN + ",itqitN," + itqitN + ",itqratioN," + itqratioN + ",itqBoundN," + itqBoundN)
    ...
    val hash = new ZFHashLayer(itqitN, itqratioN, itqBoundN, isSparse, 1, redisHost, sc)
    val objectData: RDD[(LabeledPoint, Array[LabeledPoint])] = data
      .map(lp => (lp.label, lp)).groupByKey().flatMap(t => hash.zfHash(t._2)).persist(StorageLevel.MEMORY_AND_DISK)
    val oN = objectData.count()

    println("hashTime," + (System.currentTimeMillis() - tc) + ",oN," + oN + "," + objectData.take(10).map(_._2.size).mkString(","))
    ...
    //    val ZFZipLenet5: ZFZipLenet5 = new ZFZipLenet5(topology, 10, 10).setMaxIterations(numIt).setMiniBatchSize(batchSize)
...
//    val ziplenet5: ZFZipLenet5 = new ZFZipLenet5(topology, 120, 10, inputN).setMaxIterations(numIt).setMiniBatchSize(batchSize)
    ...
    //    //    topology.addLayer(CNNLayer.buildOutputLayer(120)) //84,10
    //    val ZFZipLenet5: ZFZipLenet5 = new ZFZipLenet5(topology, 120, 10, inputN).setMaxIterations(numIt).setMiniBatchSize(batchSize)
    ...
    val ziplenet5: ZFZipLenet5 = new ZFZipLenet5(topology, 32, 10, inputN).setMaxIterations(numIt).setMiniBatchSize(batchSize)


    ziplenet5.train(objectData, test, foldN)
    val acc = ziplenet5.predictFullCollection(test)
