      ...
      val (gradientSum, right, count) = trainSet
        ...
        .treeAggregate((gZero, 0, 0))(
          seqOp = (c, v) => {
            ...
            //            val g1k: Array[BDM[Double]] = gradient(1)._1(0)
            //            val g1b: Array[Double] = gradient(1)._2
            //            val g1ksum = g1k.map(m => {
            //              m.toArray.map(e => e * e).sum
            //            }).sum
            //            val g1sum = g1ksum + g1b.map(e => e * e).sum
            //            zfg.add(g1sum)
...
object CNN {
  ...
  def main(args: Array[String]) {
    ...
    val cnn: CNN = new CNN(topology).setMaxIterations(numIt).setMiniBatchSize(batchSize)
    cnn.train(data)

    val rightN = cnn.predict(test.map(_.features)).zip(test.map(_.label)).map(t => if (t._1 == t._2) 1 else 0).sum()
