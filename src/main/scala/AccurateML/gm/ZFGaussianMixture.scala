...
/**
  ...
  */
@Since("1.3.0")
class ZFGaussianMixture private(
                                 private var k: Int,
  ...
  def run(data: RDD[Vector]): GaussianMixtureModel = {
    ...
    val shouldDistributeGaussians = ZFGaussianMixture.shouldDistributeGaussians(k, d)
    ...
    while (iter < maxIterations && math.abs(llh - llhp) > convergenceTol) {
      ...
      //      val sums=breezeData.mapPartitions(pit=>{
      //        val zero = ExpectationSum.zero(k,d)
      //        while (pit.hasNext){
      //          val x:breeze.linalg.Vector[Double] = pit.next()
      //          compute.value(zero,x)
      //        }
      //        Array(zero).toIterator
      //      }).reduce((a,b)=>{
      //        a += b
      //      })
...
object ZFGaussianMixture {
  ...
  def main(args: Array[String]) {
    ...
    for (r <- ratios) {
      ...
      val gm = new ZFGaussianMixture(k, convergenceTol, maxIterations, Utils.random.nextLong())
