...
object SVMExampleSplit01 {
  def main(args: Array[String]): Unit = {
    ...
    val splitN = 2 //args(14).toDouble
    val resultPath = args(14)
    ...
    jedis.flushAll()
    val oHash = new ZFHash(itqbitN, itqitN, itqratioN, upBound, splitN, true,redisHost)
    ...
    val fw2 = new FileWriter(resultPath.split('.')(0)+".two", false)
    fw2.append(HashMapT.sum+"\n")//"Input data aggregation  time:\t"+
    fw2.append(clusterN.sum / clusterN.size+"")//"Average number of original data points per aggregated data point:\t"+
    fw2.close()



    val rDivN = 100
    val ratioL = test100 //if (test100) List(rDivN) else List(1, 50,100)
    ...
    val fw = new FileWriter(resultPath, true)
    fw.append("ratio\tMSE\tnnMapT\n")
    fw.flush()
    for (r <- ratioL) {
      ...
      if(r!=ratioL.last)
        fw.append(ratio + "\t" + model.modelAccuracy(scoreAndLabels2) + "\t" + (t2 - t1).toDouble + "\n")
      else
        fw.append(ratio + "\t" + model.modelAccuracy(scoreAndLabels2) + "\t" + (t2 - t1).toDouble)
      fw.flush()
