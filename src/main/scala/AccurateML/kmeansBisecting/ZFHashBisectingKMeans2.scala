...
//
//import AccurateML.kmeans.ZFHashKmeans
...
//      }
//      //      println("level \t" + level+"\tnewClusterCenters"+(k-numLeafClustersNeeded))
//
//
//      level += 1
//    }
...
//      val train = splits(0)
//      val bkm = new ZFHashBisectingKMeans().setK(k)
...
//      sb.append(ratio + "\t" + zfmse + "\t" + time + "\n")
//      println("ratio" + ratio + s"\tCompute Cost: ${mse}\t" + zfmse + "\t" + time)
//      println("index\tsize\tcost")
//      println(model.root.leafNodes.map(c => Array(c.index, c.size, c.cost).mkString(",\t")).mkString("\n"))
//      println()
...
//                                 divisibleIndices: Set[Long],
//                                 newClusterCenters: Map[Long, VectorWithNorm]): RDD[(Long, VectorWithNorm)] = {
//    //    println("divisibleIndices:\t" + divisibleIndices.toArray.mkString(","))
//    //    println("newClusterCenters keys \t" + newClusterCenters.keySet.toArray.mkString(","))
//
//    assignments.map { case (index, v) =>
//      if (divisibleIndices.contains(index)) {
//        //zf
