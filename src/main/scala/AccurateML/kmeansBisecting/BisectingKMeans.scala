    ...
    while (activeClusters.nonEmpty && numLeafClustersNeeded > 0 && level < LEVEL_LIMIT) {
      ...
      if (divisibleClusters.nonEmpty) {
        ...
        var newClusterCenters = divisibleClusters.flatMap { case (index, summary) =>
          val (left, right) = ZFHashBisectingKMeans.zfsplitCenter(summary.center, random)
        ...
        var lastAssignments: RDD[(Long, VectorWithNorm)] = null
        //        println("first split")
        ...
        for (iter <- 0 until maxIterations) {
          ...
          //            val temp = lastAssignments.zip(newAssignments).filter(ab => ab._1._1 != ab._2._1)
          //            println("changed count," + temp.count())
          //            temp.takeSample(false, 20).foreach(ab => println("changed," + ab._1._1 + "\t" + ab._2._1 + "\t" + ab._1._2.vector.toArray.slice(0, 5).mkString(",")))
          //          }

          lastAssignments = newAssignments

          newClusters = summarize(d, newAssignments)
          //          println("level\t"+level+"\titer\t"+iter)
          //          newAssignments.sortBy(t => t._2.vector(0)).map(t => t._1+" , "+t._2.vector.toArray.slice(0,5).mkString(",")).foreach(s=>println(s))
          //          println(newClusters.map(t => t._1+" , "+t._2.size+" , "+t._2.center.vector.toArray.slice(0,5).mkString(",")).toArray.mkString("\t"))
        ...
        numLeafClustersNeeded -= (newClusterCenters.size - divisibleClusters.size) //k - newClusterCenters.size
        zftimesb.append(",updateIndicesT, " + (System.currentTimeMillis() - zftime))
        zftime = System.currentTimeMillis()
        //        numLeafClustersNeeded -= divisibleClusters.size//zf
      } else {
...
private object BisectingKMeans extends Serializable {
  ...
  def main(args: Array[String]) {
    ...
    for (r <- ratios) {
      ...
      val data: RDD[Vector] = if (isSparse) {
        MLUtils.loadLibSVMFile(sc, dataPath, numFeatures, minPartition).map(point => point.features)
      } else {
        sc.textFile(dataPath).map(line => Vectors.dense(line.split(",").map(_.toDouble))).cache() //"/Users/zhangfan/Documents/data/mllib/gmm_data_comma.txt"
      }


      val splits = data.randomSplit(Array(ratio, 1 - ratio))
      val train = splits(0)
      val bkm = new BisectingKMeans().setK(k).setMaxIterations(maxIterations)
      val model = bkm.run(train)
      time = System.currentTimeMillis() - time

      var timecost = System.currentTimeMillis()
      val mse = model.computeCost(data)
      //      val zfmse = ZFHashKmeans.zfComputeCost(data, model.clusterCenters)
      timecost = System.currentTimeMillis() - timecost
      sb.append(ratio + "\t" + mse + "\t" + time + "\t" + timecost + "\n")
      //      println("ratio" + ratio + s"\tCompute Cost: ${mse}\t" + zfmse + "\t" + time + "\t" + model.root.leafNodes.size + " [ " + model.root.leafNodes.map(o => o.size).mkString(","))
  ...
  private def updateAssignments(
                                 ...
                                 newClusterCenters: Map[Long, VectorWithNorm]): RDD[(Long, VectorWithNorm)] = {
    //print
    //    val pdiv = divisibleIndices.toArray.mkString(",")
    //    val pnewcenter = newClusterCenters.map(t => t._1 + " : " + t._2.vector.toArray.slice(0, 5).mkString(",")).toArray.mkString("\t")
    //    val ppoints = assignments.map(o => o._1).distinct().collect()
    //    val ppmap = assignments.map(o => o._1).map(index => (index, 1)).reduceByKey((a, b) => a + b).collect()
    //    println("div," + pdiv + "\t,newC," + pnewcenter + "\t,points," + ppoints.mkString(",") + "\tpmap" + ppmap.mkString(","))
    ...
    assignments.map { case (index, v) =>
      if (divisibleIndices.contains(index)) {
        //zf
        var children = Seq(leftChildIndex(index), rightChildIndex(index))
        children = children.filter(childi => newClusterCenters.contains(childi))

//        //p
//        val p=children.map( index => index+" : "+ KMeans.fastSquaredDistance(newClusterCenters(index),v)+" , "+dot(newClusterCenters(index).vector, v.vector)).mkString("\t")
//        println("indexDis\t"+p)
//        //end-p
