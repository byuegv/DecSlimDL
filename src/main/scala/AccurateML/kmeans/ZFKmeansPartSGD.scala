...
//
//
//class ZFKmeansPartSGD(k: Int, itN: Int, redisHost: String, sc: SparkContext) extends Serializable {
//
//  val costHis = new ArrayBuffer[Double]()
//  var mapT = sc.longAccumulator
//
//
//  def runAlgorithm(data: RDD[Vector], origCenters: Array[Vector], ratio: Double): Array[Vector] = {
//
...
//        val rand = new Random()
//        val randPoint = points.filter(vec => rand.nextDouble() <= ratio)
//        randPoint.foreach { point =>
//          val (centerIndex, cost) = ZFKmeansPartSGD.zfFindClosest(point, thisCenters)
...
//        }
//      }
//      println(it + "\trunAlgorithm MapWSSSE: " + costAccum.value + "\tcenters: " + centers.map(_.toArray.sum).sum + "\t accMapT: " + mapT.value)
...
//    val sc = data.sparkContext
//
//    val mapdata = data.map(vec => Tuple2(vec, Array(0.0))) // lastCenterIndex
...
//        }else{
//          var cenCostAB = new ArrayBuffer[Tuple2[Double, Int]]() //sortValue,zipIndex
//          var zipN = 0
...
//            val (centerIndex, cost, relastCost) = ZFKmeansPart.zfFindClosestNf(vec, thisCenters, lastCenterIndex.toInt,10)
//            val sortValue = relastCost - cost
//            cenCostAB += Tuple2(sortValue, zipIndex)
//            zipN+=1
//          }
//          cenCostAB.sortWith(_._1>_._1).map(_._2).slice(0,(zipN*ratio).toInt).toArray
//        }
//
//
//        var tempi =0
...
//            val (centerIndex, cost, relastCost) = ZFKmeansPart.zfFindClosest(vec, thisCenters, info(0).toInt)
//            val sortValue = relastCost - cost
//            info(0) = centerIndex
////            costAccum.add(cost)
//          }
...
//
//
//  def printChangeRatio(data: RDD[Vector], origCenters: Array[Vector], ratio: Double): Array[Vector] = {
//
//    if (data.getStorageLevel == StorageLevel.NONE) {
//      System.err.println("------------------The input data is not directly cached, which may hurt performance if its"
//        + " parent RDDs are also uncached.------------------------")
...
//    val selectNs = new ArrayBuffer[Double]()
//
//    var pindex = data.map(vec => {
//      var sum = 0.0
//      vec.foreachActive((index, value) => sum += value)
//      sum + ""
//    })
//
//    for (it <- 0 until itN) {
...
//      val costAccum = sc.doubleAccumulator
//
//      val iti = data.map(vec => {
//        val thisCenters = bcCenters.value
//        val (centerIndex, cost) = ZFKmeansPartSGD.zfFindClosest(vec, thisCenters)
//        centerIndex
//      })
//      pindex = pindex.zip(iti).map(t => t._1 + "," + t._2)
//      pindex.persist(StorageLevel.DISK_ONLY_2)
//      pindex.count()
//
//      val totalContibs = data.mapPartitions { points =>
//        val jedis = new Jedis(redisHost)
...
//    } //end-it
//
//    pindex.saveAsTextFile("file:/home/zhangfan/sh/kmeans/out")
//    //    pindex.saveAsTextFile("/Users/zhangfan/Downloads/cancel")
//
//    val n = data.count()
//    val changeRatio = new ArrayBuffer[Double]()
//    for (i <- 1 until itN) {
//      val tempd = pindex.filter(line => {
//        val vs = line.split(",")
//        vs(i).toInt != vs(i + 1).toInt
//      })
//      changeRatio.append(tempd.count().toDouble / n)
//    }
//    println("changeRatio:\t" + changeRatio.mkString(","))
//    pindex.unpersist()
//
//
//    println("selectNs: " + selectNs.map(_ / 10000.0).mkString(","))
...
//    val ans = data.map(point => ZFKmeansPartSGD.zfFindClosest(point, centers)._2).sum()
//    ans
//  }
//
//}
...
//
//
//      val zfkmeans = new ZFKmeansPartSGD(k, itN, redisHost, sc)
//      val centers = zfkmeans.runAlgorithm(train, origCenters, ratio)
////      val centers = zfkmeans.runAlgorithm1(train, origCenters, ratio)
//
//
