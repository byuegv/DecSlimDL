...
class EachClassNLLCriterionOne[@specialized(Float, Double) T: ClassTag]
  ...
  def updateOutput(input: Tensor[T], target: Tensor[T]): Array[T] = {
    ...
    if (input.dim() == 1) {
      ...
      while (i <= batchSize) {
        ...
        val tempRes = if (curTarget == paddingValue) (ev.zero, ev.zero)
        else {
        ...
        output(_i - 1) = ev.negative(tempRes._1)
        total_weight = ev.plus(total_weight, tempRes._2)
        i += 1
      }
...
object EachClassNLLCriterionOne {
  def apply[@specialized(Float, Double) T: ClassTag](
                                                      weights: Tensor[T] = null,
                                                      sizeAverage: Boolean = true,
                                                      logProbAsInput: Boolean = true,
                                                      paddingValue: Int = -1
                                                    )(implicit ev: TensorNumeric[T]) : EachClassNLLCriterionOne[T] = {
    new EachClassNLLCriterionOne[T](weights, sizeAverage, logProbAsInput, paddingValue)
  }
}
