...
object DecGossipGradDistriOptimizer extends AbstractOptimizer {
  import Optimizer._
  ...
  private[optim] def optimize[T: ClassTag](
                                            ...
                                            dataset: DistributedDataSet[(Long,Array[Sample[T]])],
                                            coresPerNode: Int,
                                            state: Table,
                                            endWhen: Trigger,
                                            metrics: Metrics,
    ...
    val _sapAry = dataset.data(false)
    val _ndataset = (DataSet.rdd(_sapAry) -> SampleToIDAryMiniBatch(totalBatch = goControler.batchSize))
      .asInstanceOf[DistributedDataSet[MiniBatch[T]]]

    var dataRDD = _ndataset.data(train = true)
    ...
    var numSamples = _ndataset.data(train = false).map(_.size()).reduce(_ + _)
    ...
    while (!endWhen(driverState)) {
      ...
      // =============  pull new training samples from other edges
      if (recordsProcessedThisEpoch == 0) {
        val shuffleBefore = System.nanoTime()
        ...
        dataRDD = _nxdataset.data(true)
        val shuffleEnd = System.nanoTime()
        logger.info(s"Pull data complete. Takes ${(shuffleEnd - shuffleBefore) / 1e9}s")
      }
      ...
      val numFinishedModelUpdates: Int = dataRDD
        .zipPartitions(models, preservesPartitioning = true) { (data, modelIter) => {
          ...
          if(remvControler.isExtraCompute && remvControler.isTaskRemove == true && (curEpoch > remvControler.excludeEpoch || curIteration > remvControler.excludeIteration)){
            // set to validation mode
            Engine.default.sync(tasks)
            tasks.clear()
            tasks ++= Engine.default.invoke {(0 until _subModelNumber).map { i =>
              () => {
                cached.localModels(i).evaluate()
              }
            }
            }
            Engine.default.sync(tasks)
            ...
            val filterThreads = Engine.default.invokeAndWait2((0 until _subModelNumber).map(i =>
              () => {
                ...
                if(remvControler.removeCriterion == "loss"){
                  val localEachClassNLLCriterion = (bcEachClassNLLCriterion.value)(i)
                  ...
                  if (Engine.getEngineType() == MklBlas || localModel.isInstanceOf[IRGraph[T]]) {
                    ...
                    //                    logger.info(localCriterion.forward(output,target).asInstanceOf[Float])
                    val eachloss = localEachClassNLLCriterion.forward(output,target)
                    //                    logger.info(eachloss.mkString(","))
                    ...
                    for(cu <- 0 until sampleAryLength){
                      ...
                      costInformation(i) += Tuple3(i,headArySamples(cu)._1,if(tl.isNaN || tl.isInfinite) 0.0 else tl)
                    }
                ...
                else if(remvControler.removeCriterion == "lossGrad"){
                  val lossGrad = Tensor()
                  for(cur <- 0 until sampleAryLength){
                    ...
                    if (Engine.getEngineType() == MklBlas || localModel.isInstanceOf[IRGraph[T]]) {
                      ...
                      val gsum = lossGrad.resizeAs(errors.toTensor).copy(errors.toTensor).abs().sum().asInstanceOf[Float] // get gradient
                  ...
                  val grad = Tensor[T]()
                  for(cur <- 0 until sampleAryLength){
                    val preGrad = if(!remvControler.fullGradients)localModel.getParametersTable()[Table](remvControler.layerName)[Tensor[T]](remvControler.gradName)
                    else localModel.getParameters()._2
                    ...
                    val trainGrad = if(!remvControler.fullGradients)localModel.getParametersTable()[Table](remvControler.layerName)[Tensor[T]](remvControler.gradName)
                    else localModel.getParameters()._2
                    grad.sub(trainGrad)
            ...
            val sortedCostInfo = finishedFilterThreads.map(x => costInformation(x).toArray).reduce(_ ++ _)
              .sortWith((x,y) => x._3 < y._3) // sort from small to large
            ...
            val maxLoss = sortedCostInfo.map(_._3).max
            val minLoss = sortedCostInfo.map(_._3).min
            var nonCriticalNum: Int = 0 // number of non-critical
            logger.info(s"max: ${maxLoss} min: ${minLoss} mean: ${meanCostInfo}")
            if(remvControler.taskStrategy == "fixedratio"){
              ...
              val chooseIndex = SampleUtils.reservoirMult(sapWeights,chooseNum)
              ...
              // if loss < meanCostInfo * taskRatio,treated as non-critical samples
              var critThreshold = meanCostInfo * remvControler.taskFraction
              if(remvControler.isEpochRemove){
                critThreshold = maxLoss * 0.95
              }
              else{
                if(critThreshold >= maxLoss) {critThreshold = meanCostInfo}
              }
              for(cu <- 0 until totalSortedCostInfoLength){
                if(sortedCostInfo(cu)._3 >= critThreshold || (nonCriticalNum*1.0 / totalSortedCostInfoLength) >= remvControler.limitRemoveFraction){
                  criticalIndex(sortedCostInfo(cu)._1) += sortedCostInfo(cu)._2
                }
                else{
                  nonCriticalNum += 1
            ...
            if(remvControler.isTaskRemove == true && (curEpoch > remvControler.excludeEpoch || curIteration > remvControler.excludeIteration)) {
              ...
              val xmod = Math.max(1,totalCriticalSampleLength - 1)
              var xoffset: Int = 0
              for (cu <- 0 until _subModelNumber) {
                val tpSapAry = criticalSample.slice(xoffset % xmod, (xoffset % xmod) + perUnit)
           ...
           copy the weights from parameter server into the first model's weights.
          */
            val weightsResultsRef = parameters.values.map(p =>
              p.getWeights(cached.modelWeights.head.narrow(1, p.paramOffset, p.size))
            ).toArray
            weightsResultsRef.foreach(_.waitResult())
          }
          ...
          val trainingThreads = Engine.default.invokeAndWait2((0 until _subModelNumber).map(i =>
            () => {
              ...
              localModel.training()
              val localCriterion = cached.localCriterions(i)
              val idminiBatch = miniBatchBuffer(i).asInstanceOf[IDAryMiniBatch[T]] // change to IDAryMiniBatch
              val miniBatch = if(remvControler.isAggregatedSamples == false || idminiBatch.isAggregated() == false){idminiBatch} else{
                val tsaps = idminiBatch.getHeadSamples().map(_._2)
                SampleToMiniBatch(tsaps.length,partitionNum = Some(1)).apply(tsaps.toIterator).next()
              }
      ...
      if (dropPercentage == 0.0 ||
        ...
        // ++++++++++++++ push gradients and pull gradients ++++++++++++++++++++
        logger.info(s"${edgeName} pull latest model to driver")
        getModel(models, parameters, trainingModel) // pull the latest model parameters to driver
        val (_,grd) = trainingModel.getParameters()
        val oEdgeId = (edgeID + math.pow(2,iteration)).toInt % goControler.edgeNum + 1
        jedisHelper.lpushGradients(grd,Some(oEdgeId))

        val ograd = jedisHelper.brpopGradients[T]()
        val bograd = sc.broadcast(ograd)
        ...
        models.mapPartitions { modelIter =>
          ...
          modelCache.optimMethods.foreach{ case (name, optimMethod) =>
            ...
            // merge gradients
            p.gradientPartition.add(oxgrad)
            p.gradientPartition.asInstanceOf[Tensor[Float]].mul(0.5f)
...
class DecGossipGradDistriOptimizer[T: ClassTag] (
                                          _model: Module[T],
                                          _dataset: DistributedDataSet[(Long,Array[Sample[T]])],
                                          _criterion: Criterion[T]
                                        )(implicit ev: TensorNumeric[T])
  extends Optimizer[T, Tuple2[Long,Array[Sample[T]]]](
   ...
   */
  def clearState() : Unit = {
    DecGossipGradDistriOptimizer.clearState(models)
  }
  ...
  private def endEpoch(): Unit = {
    DecGossipGradDistriOptimizer.endEpoch(optimMethods)
  }

  def setTrainData(sampleRDD: RDD[(Long,Array[Sample[T]])],
                   batchSize: Int): this.type = {
    this.dataset = (com.intel.analytics.bigdl.dataset.DataSet.rdd(sampleRDD)
      -> com.intel.analytics.bigdl.dataset.Identity()).asInstanceOf[DistributedDataSet[(Long,Array[Sample[T]])]]
  ...
  private[bigdl] def prepareInput2[T: ClassTag](dataset: DataSet[(Long,Array[Sample[T]])],
                                                validationDataSet: Option[DataSet[MiniBatch[T]]]): Unit = {
    dataset.asInstanceOf[DistributedDataSet[(Long,Array[Sample[T]])]].cache()
  ...
  override def prepareInput(): Unit = {
    if (!dataset.toDistributed().isCached) {
      DecGoSGDDistriOptimizer.logger.info("caching training rdd ...")
  ...
  override def optimize(): Module[T] = {
    ...
    val modelsAndBroadcast = DecGossipGradDistriOptimizer.initThreadModels(trainingModel, distDataset, criterion,
    ...
    while (retryNum < maxRetry) {
      try {
        DecGossipGradDistriOptimizer.optimize(
          trainingModel,
          distDataset,
          coresPerNode,
          state,
        ...
        case e: IllegalArgumentException =>
          throw e
        case t: Throwable =>
          DecGossipGradDistriOptimizer.logger.error("Error: " + ExceptionUtils.getStackTrace(t))
          if (checkpointPath.isDefined) {
            ...
            DecGossipGradDistriOptimizer.logger.info(s"Retrying $retryNum times")
            ...
            val newModel = if (modelFile != null) {
              DecGossipGradDistriOptimizer.logger.info("Model recover from last snapshot")
              Module.load[T](modelFile)
            } else {
              DecGossipGradDistriOptimizer.logger.info("Model recover from origin model")
              trainingModel
            }
            optimMethods = optimMethods.map { case (moduleName, optimMethod) =>
              ...
              val newOptimMethod = if (methodFile != null) {
                DecGossipGradDistriOptimizer.logger.info(s"$moduleName's OptimMethod recover from last snapshot")
                OptimMethod.load[T](methodFile)
              } else {
                DecGossipGradDistriOptimizer.logger.info(s"$moduleName's OptimMethod recover from origin model")
                optimMethod
              }
              newOptimMethod.clearHistory()
              (moduleName, newOptimMethod)
            }
            val modelsAndBroadcast = DecGossipGradDistriOptimizer.initThreadModels(newModel, distDataset,
    ...
    DecGossipGradDistriOptimizer.getModel(models, parameters, trainingModel)
