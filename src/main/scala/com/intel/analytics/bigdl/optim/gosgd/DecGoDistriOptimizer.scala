...
object DecGoDistriOptimizer extends AbstractOptimizer {
  import Optimizer._
...
class DecGoDistriOptimizer[T: ClassTag] (
                                          _model: Module[T],
   ...
   */
  def clearState() : Unit = {
    DecGoDistriOptimizer.clearState(models)
  }
  ...
  private def endEpoch(): Unit = {
    DecGoDistriOptimizer.endEpoch(optimMethods)
  }
  ...
  override def prepareInput(): Unit = {
    if (!dataset.toDistributed().isCached) {
      DecGoDistriOptimizer.logger.info("caching training rdd ...")
  ...
  override def optimize(): Module[T] = {
    ...
    val modelsAndBroadcast = DecGoDistriOptimizer.initThreadModels(trainingModel, distDataset, criterion,
    ...
    while (retryNum < maxRetry) {
      try {
        ...
        case e: IllegalArgumentException =>
          throw e
        case t: Throwable =>
          DecGoDistriOptimizer.logger.error("Error: " + ExceptionUtils.getStackTrace(t))
          if (checkpointPath.isDefined) {
            ...
            DecGoDistriOptimizer.logger.info(s"Retrying $retryNum times")
            ...
            val newModel = if (modelFile != null) {
              DecGoDistriOptimizer.logger.info("Model recover from last snapshot")
              Module.load[T](modelFile)
            } else {
              DecGoDistriOptimizer.logger.info("Model recover from origin model")
              trainingModel
            }
            optimMethods = optimMethods.map { case (moduleName, optimMethod) =>
              ...
              val newOptimMethod = if (methodFile != null) {
                DecGoDistriOptimizer.logger.info(s"$moduleName's OptimMethod recover from last snapshot")
                OptimMethod.load[T](methodFile)
              } else {
                DecGoDistriOptimizer.logger.info(s"$moduleName's OptimMethod recover from origin model")
                optimMethod
              }
              newOptimMethod.clearHistory()
              (moduleName, newOptimMethod)
            }
            val modelsAndBroadcast = DecGoDistriOptimizer.initThreadModels(newModel, distDataset,
    ...
    DecGoDistriOptimizer.getModel(models, parameters, trainingModel)
