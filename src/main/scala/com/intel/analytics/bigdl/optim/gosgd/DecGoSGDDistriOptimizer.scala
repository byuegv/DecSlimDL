...
object DecGoSGDDistriOptimizer extends AbstractOptimizer {
  import Optimizer._
  ...
  private[optim] def optimize[T: ClassTag](
    ...
    val jedisHelper = new GoSGDHelper(host = goControler.redisHost,port = goControler.redisPort,
    ...
    var paramScore: Double = 1.0 / goControler.edgeNum  // The initial score of current parameters
    ...
    while (!endWhen(driverState)) {
      ...
      if (dropPercentage == 0.0 ||
        ...
        if (recordsProcessedThisEpoch >= numSamples) {
          ...
          logger.info(s"${_header} Save epoch cost time of ${edgeName} to Redis: ${curEpochTimeCost} s")
          epochCostStart = System.nanoTime()

          val mergeModelWeightsStart = System.nanoTime()
          logger.info(s"${_header} ${edgeName} pull latest model to driver")
          ...
          if(threshold < goControler.exchangeProb){
            jedisHelper.insertoTime(curEpochTimeCost)
          }
          else{
            System.out.println(("Insert timecost and parameters"))
            paramScore = paramScore / 2.0
            jedisHelper.insertScoreWeightxTime[T](curEpochTimeCost,selId,paramScore,paramsTensor)
          }
          logger.info(s"${_header} ${edgeName} Save latest model parameters to Resis")
          ...
          val (weightScoreAry, epochTimex) = jedisHelper.popScoreWeightxTime[T]()
          ...
          logger.info(s"${_header} ${edgeName} pull epoch time finished: min ${minEpochTimeCost} s, max ${maxEpochTimeCost} s " +
            s"criticalThreshold: ${criticalThreshold}")

          logger.info(s"${_header} weightScoreAry: ${weightScoreAry.length}, scores: ${weightScoreAry.map(_._1).mkString(",")}")

          val broadScore = sc.broadcast(paramScore)
          ...
          models.mapPartitions { modelIter =>
            ...
            modelCache.optimMethods.foreach{ case (name, optimMethod) =>
              var time = System.nanoTime()
              val p = parameters(name)
              var localScore = broadScore.value
              weightScoreAry.foreach(scoreWeight => {
                val score = scoreWeight._1
                val otherEdgeParamsTensor = scoreWeight._2

                var w1 = (localScore / (score + localScore)).toFloat
                var w2 = (score / (score + localScore)).toFloat
                localScore = localScore + score

                // not use sum-weight protocol
                //w1 = 0.5f
                //w2 = 0.5f
                ...
                p.weightPartition.asInstanceOf[Tensor[Float]].mul(w1)
                otherEdgeParamsTensor.asInstanceOf[Tensor[Float]].mul(w2)
                p.weightPartition.asInstanceOf[Tensor[Float]].add(otherEdgeParamsTensor.asInstanceOf[Tensor[Float]])
          ...
          paramScore = paramScore + weightScoreAry.map(_._1).sum
          broadScore.destroy()

          logger.info(s"${_header} current weight score of ${edgeName}: ${paramScore}")
...
class DecGoSGDDistriOptimizer[T: ClassTag] (
                                          _model: Module[T],
   ...
   */
  def clearState() : Unit = {
    DecGoSGDDistriOptimizer.clearState(models)
  }
  ...
  private def endEpoch(): Unit = {
    DecGoSGDDistriOptimizer.endEpoch(optimMethods)
  }
  ...
  override def optimize(): Module[T] = {
    ...
    val modelsAndBroadcast = DecGoSGDDistriOptimizer.initThreadModels(trainingModel, distDataset, criterion,
    ...
    while (retryNum < maxRetry) {
      try {
        DecGoSGDDistriOptimizer.optimize(
          trainingModel,
          distDataset,
          coresPerNode,
          state,
        ...
        case e: IllegalArgumentException =>
          throw e
        case t: Throwable =>
          DecGoSGDDistriOptimizer.logger.error("Error: " + ExceptionUtils.getStackTrace(t))
          if (checkpointPath.isDefined) {
            ...
            DecGoSGDDistriOptimizer.logger.info(s"Retrying $retryNum times")
            ...
            val newModel = if (modelFile != null) {
              DecGoSGDDistriOptimizer.logger.info("Model recover from last snapshot")
              Module.load[T](modelFile)
            } else {
              DecGoSGDDistriOptimizer.logger.info("Model recover from origin model")
              trainingModel
            }
            optimMethods = optimMethods.map { case (moduleName, optimMethod) =>
              ...
              val newOptimMethod = if (methodFile != null) {
                DecGoSGDDistriOptimizer.logger.info(s"$moduleName's OptimMethod recover from last snapshot")
                OptimMethod.load[T](methodFile)
              } else {
                DecGoSGDDistriOptimizer.logger.info(s"$moduleName's OptimMethod recover from origin model")
                optimMethod
              }
              newOptimMethod.clearHistory()
              (moduleName, newOptimMethod)
            }
            val modelsAndBroadcast = DecGoSGDDistriOptimizer.initThreadModels(newModel, distDataset,
    ...
    DecGoSGDDistriOptimizer.getModel(models, parameters, trainingModel)
